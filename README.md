...
市面上的 AI 图像工具都是**单图抽卡**模式——输入一句提示词，等待一张图。漫画需要的东西多得多：**角色 × 风格 × 叙事 × 分镜 × 构图 × 迭代**，一条完整的工程链。

MangaZine 把这条链路拆成**4 个专职 Agent + 1 套编排引擎**，每个环节可检查、可回退、可迭代：
...
用**有限状态机 + 事件驱动**替代一个 500 行的 `async` 函数：
...
替掉之前 `child_process.spawn` 的临时方案，换成独立的 Python API 服务：
